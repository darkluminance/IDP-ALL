...
def connect_the_dots(points):
    ...
    #     for j in rannonlocalge(len(arr[i])):
    #         if arr[i][j] == 0:
    #             dotList.append([i, j, 0])
    ...
    # for pp in range(len(distList)):
    #     jj = list(distList.values())[pp]
    #     print(jj)
    #     if(not visited[jj[0]]):
    #         dfs(jj[0])
    ...
    # print(cc)
    lasto = (sender2[0], sender[0])
    for i in range(len(sender)):
        # print("G01 X", sender2[i], " Y", sender[i])
        # print(f"process(F(\"G01 X{sender2[i]/3} Y{sender[i]/3}\"));")
        if i > 0 and sender[i-1] == -1:
            lasto = (sender2[i], sender[i])
        if i == len(sender) - 1:
            print("", lasto[0], " ", lasto[1])
            print("", lasto[0], " ", lasto[1])
            print("", lasto[0], " ", lasto[1])
        print("", sender2[i], " ", sender[i])
    # print(sender)
    # print(sender2)
# connect_the_dots()